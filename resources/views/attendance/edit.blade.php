...
@section('content')
    <div class="page-header">
        <h1><i class="bi bi-geo-alt" style="margin-right: 8px;"></i>Edit Attendance Record</h1>
        <p>Update your attendance</p>
    </div>

    <div class="card" style="max-width: 600px; margin: 0 auto;">
        <form method="POST" action="{{ route('attendance.update', $attendance) }}" style="display: flex; flex-direction: column; gap: 20px;">
            @csrf
            @method('PUT')

            <div>
                <label for="date" style="display: block; margin-bottom: 8px; font-weight: 600;">Date <span style="color: #dc2626;">*</span></label>
                <input type="date" id="date" name="date" value="{{ old('date', optional($attendance->attendance_date)->format('Y-m-d')) }}" required style="width: 100%; padding: 10px; border: 2px solid #e5e7eb; border-radius: 6px;">
            ...
            <div>
                ...
                <input type="time" id="check_in_time" name="check_in_time" value="{{ old('check_in_time', $attendance->check_in_time) }}" required style="width: 100%; padding: 10px; border: 2px solid #e5e7eb; border-radius: 6px;">
            ...
            <div>
                ...
                <input type="time" id="check_out_time" name="check_out_time" value="{{ old('check_out_time', $attendance->check_out_time) }}" style="width: 100%; padding: 10px; border: 2px solid #e5e7eb; border-radius: 6px;">
            ...
            <div>
                ...
                <input type="number" id="latitude" name="latitude" value="{{ old('latitude', $attendance->check_in_latitude) }}" step="0.0001" style="width: 100%; padding: 10px; border: 2px solid #e5e7eb; border-radius: 6px;">
            ...
            <div>
                ...
                <input type="number" id="longitude" name="longitude" value="{{ old('longitude', $attendance->check_in_longitude) }}" step="0.0001" style="width: 100%; padding: 10px; border: 2px solid #e5e7eb; border-radius: 6px;">
            ...
            <div>
                ...
                <textarea id="notes" name="notes" rows="4" style="width: 100%; padding: 10px; border: 2px solid #e5e7eb; border-radius: 6px; font-family: inherit;">{{ old('notes', $attendance->notes) }}</textarea>
                @error('notes') <span style="color: #dc2626; font-size: 12px;">{{ $message }}</span> @enderror
            </div>

            <div style="display: flex; gap: 10px;">
                <button type="submit" class="btn" style="flex: 1;">Update Record</button>
